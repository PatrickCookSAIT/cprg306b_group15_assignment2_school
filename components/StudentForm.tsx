...
//reusable class for all input field boxes
...
const StudentForm = () => {
    ...
    //track state of submittedStudent, i.e. if new student data has been submitted.
    // begins null, changes when student registration form is submitted. Resets upon selecting to register a new student
    const [submittedStudent, setSubmittedStudent] =
    useState<StudentRegistrationData | null>(null);

    //default value for student forms
    ...
    if(isSubmitSuccessful && submittedStudent){
        //displays firstName and lastName of a successful student submission. Includes button to return to registration form for a new submission
        return(
    ...
    //display when first loaded. contains registration form for new students. displays again when user selects registering a new student
    return (
         <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 mt-10">
            ...
            {/* text submission for firstName. per schema, requires at least one letter */}
            <div className="flex flex-row justify-around mx-100">
            <FormField label="First Name" error={errors.firstName?.message} required>
                <input className={inputClassStyle} {...register("firstName")}/>

            </FormField>
            {/* text submission for lastName. per schema, requires at least one letter */}
            <FormField label="Last Name" error={errors.lastName?.message} required>
                <input className={inputClassStyle} {...register("lastName")}/>

            </FormField>
            </div>
            {/* submission for date. default is 2000-01-01. Format of YYYY-MM-DD. Date selected through calendar selection */}
            <div className="flex flex-row justify-around mx-100">
            <FormField label="Date of birth" error={errors.dob?.message} required >
                <input type="date" className={inputClassStyle} {...register("dob")}/>

            </FormField>
            {/*drop down selection for letter grade. selections from A+ to F and leave blank for no selection*/}
            ...
            {/* submit button. Submits if required data (firstName, lastName, dob) is complete and fulfills requirements*/}
